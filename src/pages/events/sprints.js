  ...
  render() {
    ...
    return (
      <Layout>
        ...
        <h1>{pageTitle}</h1>
        <p>For more information on sprints or to submit your own, see: <a href="/events/overview#sprints">Events Overview: Sprints</a></p>
        {sprintList.map(({ node: sprint }) => (
          <div key={sprint.id}>
            <h2>{sprint.name}</h2>
            <p>Organized by: {sprint.organizers.join(", ")}</p>
            <p>Times: {sprint.when}</p>
            <p>URL: <a href={sprint.project_url}>{sprint.project_url}</a> (<a href={sprint.getting_started_url}>getting started</a>)</p>
