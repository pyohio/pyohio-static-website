---
title: Jamal Hansen
page_type: speaker
code: RMSFVE
speaker_type: speaker
listed: true
talks:
  - code: H9EVQF
    slug: go-hybrid-find-the-results-like-can-t
    title: Go Hybrid - Find the Results LIKE Can't
    type: 30 Minute Talk
    speakers:
      - name: Jamal Hansen
        slug: jamal-hansen
        code: RMSFVE
        avatar: /2026/img/speakers/jamal-hansen.webp
social_links:
  - url: https://jamalhansen.com
    display: jamalhansen.com
    icon: mdi:link-variant
  - url: https://mastodon.online/@ham_jansen
    display: '@ham_jansen@mastodon.online'
    icon: mdi:mastodon
  - url: https://www.linkedin.com/in/jamalhansen/
    display: linkedin.com/in/jamalhansen/
    icon: mdi:linkedin
  - url: https://bsky.app/profile/ham-jansen.bsky.social
    display: '@ham-jansen.bsky.social'
    icon: mdi:bluesky
  - url: https://x.com/jamahans
    display: '@jamahans'
    icon: mdi:twitter
avatar: /2026/img/speakers/jamal-hansen.webp
og_image: https://www.pyohio.org/2026/img/og/speakers/jamal-hansen.png
---

I am a data scientist who writes to work through ideas. I enjoy building things and showing others what I've learned. Right now, I am exploring what happens when you vibe-code twenty-plus local AI tools, and I am writing a series that shows why learning SQL will help you be a better Python developer. I live in San Antonio, Texas, with my family and our dog, Butterscotch.
