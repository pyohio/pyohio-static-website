...
INDIVIDUAL_SPONSORS_URL = (
    "https://dashboard.pyohio.org/public/events/2025/individual-sponsors"
)
...
def download_logo(logo_url, filename, verbose=False):
    ...
    try:
        ...
        if verbose:
            click.echo(
                f"DEBUG: Download successful, relative path: {relative_path}", err=True
            )

        return relative_path
    except httpx.HTTPStatusError as e:
        click.echo(
            f"HTTP Error downloading logo {logo_url}: {e.response.status_code}",
            err=True,
        )
        return None
...
def get_individual_sponsors(ctx, verbose):
    ...
    try:
        ...
        click.echo(
            f"HTTP Error {sponsor_list.status_code}: {sponsor_list.text}", err=True
        )
        raise
    except Exception as e:
...
@click.option("-v", "--verbose", is_flag=True, help="Enable verbose debug output")
@click.option(
    "--skip-images", is_flag=True, help="Skip downloading sponsor logo images"
)
@click.pass_context
def get_sponsors(ctx, verbose, skip_images):
    ...
    for i, sponsor in enumerate(sponsors):
        ...
        click.echo(
            f"Processing sponsor {i + 1}/{len(sponsors)}: {sponsor_name}", err=True
        )
        ...
        if verbose:
            ...
            click.echo(
                f"DEBUG: Sponsor tier: {sponsor.get('tier', {}).get('name', 'Unknown')}",
                err=True,
            )
        ...
        if not skip_images:
            ...
            if sponsor.get("logo_light", {}).get("file_url"):
                if verbose:
                    click.echo(
                        f"DEBUG: Processing light logo for {sponsor_name}", err=True
                    )
                ...
                if "file_name" in sponsor["logo_light"]:
                    ...
                    if verbose:
                        click.echo(
                            f"DEBUG: Light logo extension from file_name: {light_ext}",
                            err=True,
                        )
                else:
                    ...
                    if verbose:
                        click.echo(
                            f"DEBUG: Light logo extension from URL: {light_ext}",
                            err=True,
                        )
                ...
                local_light_path = download_logo(
                    sponsor["logo_light"]["file_url"], light_filename, verbose
                )
                if local_light_path:
                    sponsor["logo_light"]["local_path"] = local_light_path
                    if verbose:
                        click.echo(
                            f"DEBUG: Light logo saved to: {local_light_path}", err=True
                        )
            else:
                if verbose:
                    click.echo(
                        f"DEBUG: No light logo found for {sponsor_name}", err=True
                    )

            if sponsor.get("logo_dark", {}).get("file_url"):
                if verbose:
                    click.echo(
                        f"DEBUG: Processing dark logo for {sponsor_name}", err=True
                    )
                ...
                if "file_name" in sponsor["logo_dark"]:
                    ...
                    if verbose:
                        click.echo(
                            f"DEBUG: Dark logo extension from file_name: {dark_ext}",
                            err=True,
                        )
                else:
                    # Fallback to extracting from URL
                    ...
                    if verbose:
                        click.echo(
                            f"DEBUG: Dark logo extension from URL: {dark_ext}", err=True
                        )

                dark_filename = f"{sponsor['slug']}-dark.{dark_ext}"
                click.echo(f"  Downloading dark logo: {dark_filename}", err=True)

                local_dark_path = download_logo(
                    sponsor["logo_dark"]["file_url"], dark_filename, verbose
                )
                if local_dark_path:
                    sponsor["logo_dark"]["local_path"] = local_dark_path
                    if verbose:
                        click.echo(
                            f"DEBUG: Dark logo saved to: {local_dark_path}", err=True
                        )
            else:
                if verbose:
                    click.echo(
                        f"DEBUG: No dark logo found for {sponsor_name}", err=True
                    )
        else:
    ...
    click.echo(
        f"Corporate sponsors processing complete! Processed {len(sponsors)} sponsors",
        err=True,
    )
